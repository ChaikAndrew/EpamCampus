...
public class ProposalService(IUnitOfWork unitOfWork, IMapper mapper, IAzureBlobService blobService) : IProposalService
{
    ...
    public async Task<DataPage<ProposalDto>> GetAllProposalsAsync(HttpRequest request)
    {
        ...
        // Отримання пропозицій з репозиторію з використанням OData параметрів і отримання загальної кількості
        var result = await unitOfWork.Proposals.GetProposalsAsync(false, queryOptions);
        var proposals = result.Proposals;  // Отримуємо пропозиції з кортежу
        var totalCount = result.TotalCount;  // Отримуємо загальну кількість з кортежу

        // Перетворення до DTO
        var proposalDtos = mapper.Map<IEnumerable<ProposalDto>>(proposals);

        // Створення об'єкту DataPage для посторінкового повернення
        var dataPage = new DataPage<ProposalDto>(proposalDtos.ToList(), totalCount);
